    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        switch (error.response?.status) {
          ...
          case 429:
            throw new Error("Too Many Requests (429): Too many requests have been sent in a given amount of time.");
          case 500:
            throw new Error("Internal Server Error (500): The server encountered an unexpected condition.");
          default:
            throw new Error(`Unhandled Error (HTTP ${error.response?.status}): ${error.response?.statusText}`);
        }
      } else {
...
async search(userId: User['id'], query: string, types: ('album' | 'artist' | 'track')[], limit: number = 5) {
  const authProvider = await AuthProviders.query().where('user_id', userId).first()
